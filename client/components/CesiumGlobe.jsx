...
// All available pinpoint locations with coordinates
const ALL_LOCATIONS = [
...
// Estimate latency from distance: ~0.01ms per km (fiber) + base jitter
...
// Build locations with dynamic latency from a given origin
function buildLocationsFromOrigin(originLoc) {
    return ALL_LOCATIONS
        .filter((loc) => loc.name !== originLoc.name)
        .map((loc) => {
            const distKm = haversineKm(originLoc.lat, originLoc.lon, loc.lat, loc.lon);
            const latency = estimateLatency(distKm);
            const { totalHits, successHits } = simulateHits(distKm);
            return { ...loc, latency, totalHits, successHits };
        });
}
...
export default function CesiumGlobe({ originLocation }) {
    ...
    useEffect(() => {
        ...
        // Resolve the origin from prop or default to Bangalore
        console.log('[CesiumGlobe] originLocation prop:', originLocation);
        const originLoc = originLocation
            ? ALL_LOCATIONS.find(
                (l) => l.name.toLowerCase() === originLocation.toLowerCase()
              )
            : null;

        if (!originLoc) {
            console.warn('[CesiumGlobe] Could not find location for:', originLocation, '— falling back to Bangalore');
        }

        const resolvedOrigin = originLoc || ALL_LOCATIONS[0];
        const origin = { lat: resolvedOrigin.lat, lon: resolvedOrigin.lon, name: `Origin: ${resolvedOrigin.name}` };
        const locations = buildLocationsFromOrigin(resolvedOrigin);
